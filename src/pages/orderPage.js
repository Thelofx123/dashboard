    ...
    >
      ...
      <Grid
        ...
        <Box sx={{ minWidth: '300px', background: '#F5F5F7', height: '80vh', marginTop: '20px' }} >
          ...
          {data.map((e, l) => e.type === "Захиалга" && e.date === options[selectedIndex] ? <AccordWrap orderList={orderList} data={e} date={options[selectedIndex]} key={l} type={"Захиалга"}></AccordWrap> : null)}
        </Box>
        <Divider orientation='vertical' />
        <Box sx={{ minWidth: '300px', background: '#F5F5F7', height: '80vh', marginTop: '20px' }} >
          <Typography sx={{ minWidth: "300px", border: '1px solid grey' }}>Савалсан</Typography>
          {data.map((e, l) => e.type === "Савалсан" && e.date === options[selectedIndex] ? <AccordWrap orderList={orderList} data={e} date={options[selectedIndex]} key={l} type={"Савалсан"}></AccordWrap> : null)}
        </Box>
        <Divider orientation='vertical' />
        <Box sx={{ minWidth: '300px', background: '#F5F5F7', height: '80vh', marginTop: '20px' }} >
          <Typography sx={{ minWidth: "300px", border: '1px solid grey' }}>Хүргэсэн</Typography>
          {data.map((e, l) => e.type === "Хүргэсэн" && e.date === options[selectedIndex] ? <AccordWrap orderList={orderList} data={e} date={options[selectedIndex]} key={l} type={"Хүргэсэн"}></AccordWrap> : null)}
        </Box>
        <Divider orientation='vertical' />
        <Box sx={{ minWidth: '300px', background: '#F5F5F7', height: '80vh', marginTop: '20px' }} >
          <Typography sx={{ minWidth: '300px', border: '1px solid grey', }}>Алдаатай</Typography>
          {data.map((e, l) => e.type === "Алдаатай" && e.date === options[selectedIndex] ? <AccordWrap orderList={orderList} data={e} date={options[selectedIndex]} key={l} type={"Алдаатай"}></AccordWrap> : null)}
        </Box>
